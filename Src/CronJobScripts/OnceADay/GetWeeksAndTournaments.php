...
removeOldData($connection, $currentYear);
...
function getData($connection, $year, $currentWeek, $startDate) {
	...
	if ($numTournaments !== count($dates)) {
		echo "Tournament and date counts do not match.";
		return;
	}
	...
	for ($i = 0; $i < $numTournaments; $i ++) {
		...
		if ($startDate == null || $unixDate >= $startDate) {
			...
			$row = mysqli_fetch_assoc(mysqli_query($connection, "SELECT Week FROM Tournaments WHERE Name='" . $tournaments[$i] . "' AND Year=" . $year));
			if ($row == null) {
	...
	while ($row = mysqli_fetch_assoc($databaseTournaments) != null) {
...
function removeOldData($connection, $year) {
